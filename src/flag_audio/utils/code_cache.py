...
def cache_dir_path() -> Path:
    """Return the cache directory for generated files in flagaudio."""
    _cache_dir = os.environ.get("FLAGGEMS_CACHE_DIR")
    if _cache_dir is None:
        _cache_dir = Path.home() / ".flagaudio"
    else:
...
def cache_dir() -> Path:
    """Return cache directory for generated files in flagaudio. Create it if it does not exist."""
