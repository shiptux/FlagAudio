...
from flag_audio.utils.code_cache import code_cache_dir
from flag_audio.utils.code_utils import IndentedBuffer, write_atomic
from flag_audio.utils.codegen_config_utils import CodeGenConfig, get_codegen_config
from flag_audio.utils.device_info import get_device_capability
...
from flag_audio.utils.tensor_wrapper import StridedBuffer
from flag_audio.utils.type_utils import ELEMENTWISE_TYPE_PROMOTION_KIND, type_promotion
...
class ModuleGenerator:
    ...
    def generate_imports(code: IndentedBuffer) -> IndentedBuffer:
        ...
        code.newline()
        code.writeline("from flag_audio.utils.shape_utils import (")
        code.writeline("    heuristics_for_tile_size,")
        code.writeline("    heuristics_for_num_warps,")
        code.writeline("    stride_order,")
        code.writeline(")")
        code.writeline("from flag_audio.utils.tensor_wrapper import StridedBuffer")
        code.writeline("from flag_audio.utils.libentry import libentry")
        code.writeline("from flag_audio.utils import triton_lang_extension as tle")
        code.writeline("from flag_audio.runtime import torch_device_fn")
        code.newline()
        code.newline()
        return code
...
class PointwiseDynamicFunction:
    ...
    The generated code are written out to the cache directory (defaults to ~/.flagaudio).
    """
