...
try:  # pragma: no cover - best effort fallback
    from flag_audio import runtime as _runtime
...
# set FLAGAUDIO_SOURCE_DIR for cpp extension to find
os.environ["FLAGAUDIO_SOURCE_DIR"] = str(Path(__file__).parent.resolve())

try:
    from flag_audio import c_operators
...
if use_env_c_extension and not has_c_extension:
    warnings.warn(
        "[FlagAudio] USE_C_EXTENSION is set, but C extension is not available. "
...
if has_c_extension and use_env_c_extension:
    try:
        from flag_audio import aten_patch
